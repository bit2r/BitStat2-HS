...
## 6. 로컬호스트 vs 실제 네트워크 — 측정 시 반드시 구분 (실측)

개발 중 `localhost:4848`(Quarto 프리뷰)에서 "빠르다"고 느끼는 것은 **캐시 효과가
아니라 네트워크가 아예 없기 때문**이다. 실측으로 확인했다.

```
GET .../pyodide.asm.wasm  (10,105,545 bytes)
→ 총 0.009 초, 약 1.1 GB/s (루프백/디스크 속도)
→ 응답 헤더: cache-control: no-store, max-age=0
```

두 가지 함의:

1. **다운로드가 사실상 0초** — 로컬에선 30 MB든 뭐든 즉시 온다. 그래서 §1~§5의
   "캐시로 다운로드 절약"이라는 이야기 자체가 로컬에선 **적용될 이유가 없다**(어차피 즉시).
2. **프리뷰는 `no-store`** — 브라우저가 캐시를 아예 하지 않게 지시한다. 즉 로컬 프리뷰에선
   페이지마다 에셋을 다시 받지만 9 ms라 티가 안 난다. (배포판 GitHub Pages는 `no-store`가
   아니라 실제 캐시 헤더를 주므로, **재방문 캐시 효과는 배포 환경에서만 실측된다**.)

따라서 **로컬에서 느끼는 지연은 전부 '초기화'** 다 — WASM 컴파일(9.6 MB) + Pyodide 부팅 +
`shiny`·`numpy`·`matplotlib` import + matplotlib 폰트 캐시 구축. 이는 **CPU 시간**이라
페이지마다 반복되고 캐시로 사라지지 않는다.

### 6.1 실측 (Playwright + 브라우저 성능 API)

`freq_table`(matplotlib 모듈)을 localhost에서 실제 로드해 측정한 결과:

- **첫 플롯까지 걸린 시간 ≈ 21초** — 다운로드가 사실상 0초인데도 그렇다.
  즉 이 21초는 **전부 초기화**(WASM 컴파일 + Pyodide 부팅 + import + 첫 렌더)다.
- **실제 요청 자산 = 50개, 총 34.9 MB** (아래). 이는 이전 추정(29.7 MB)보다 큰데,
  **matplotlib과 shiny가 각각 방대한 의존성 트리를 끌고 오기 때문**이다
  (contourpy·kiwisolver·pillow·fonttools·dateutil·pytz / starlette·anyio·orjson·shinychat·openssl 등).

| 자산 그룹 | 크기 | 예 |
|---|---:|---|
| Pyodide 코어(wasm+stdlib+asm.js) | 13.2 MB | pyodide.asm.wasm 9.6 |
| 패키지 휠 38개 | 19.7 MB | matplotlib 6.4 · shiny 3.8 · numpy 2.9 · openssl 1.9 · fonttools 1.0 · pillow 0.9 |
| 프론트엔드 JS/CSS/폰트 | 2.0 MB | shinylive.js · SourceSansPro woff2 |
| **합계** | **34.9 MB** | **자산 50개** |

**같은 34.9 MB를 네트워크 속도별로 환산한 다운로드 시간(계산값):**

| 회선 | 다운로드 | ＋초기화(실측) | 첫 플롯까지(합) |
|---|---:|---:|---:|
| **localhost** | ≈ 0초 | ≈ 21초 | **≈ 21초** |
| 20 Mbps | 14.0초 | ≈ 21초 | ≈ 35초 |
| 10 Mbps (Slow 4G) | 27.9초 | ≈ 21초 | **≈ 49초** |
| 1.5 Mbps (3G) | 186초 | ≈ 21초 | ≈ 3.5분 |

> ⚠️ **측정 함정:** localhost는 21초(초기화만) 보이지만, 실사용자는 여기에 **다운로드
> 시간이 더해진다** — Slow 4G면 첫 플롯까지 **약 49초**. localhost로만 보면 이 차이를
> 전혀 못 느껴 성능을 과대평가한다. 성능 판단은 반드시 **DevTools 네트워크
> 스로틀링**이나 **배포판**에서 해야 한다.
>
> 또한 재초기화 ~21초는 **캐시로도 사라지지 않고 페이지마다 반복**되므로, 다운로드를
> 캐시로 아낀 재방문·후속 페이지도 "즉시"가 아니라 **매번 ~21초 부팅**을 겪는다
> (이 사이트에서 초기화 비용이 다운로드만큼, 혹은 그 이상으로 큰 병목임을 보여 준다).

---

## 7. 결론 & "나머지도 정말 빠르게" 만들려면
