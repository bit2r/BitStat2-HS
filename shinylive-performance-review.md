...
> ✅ **권고 ①(+②) 구현 완료 (2026-07-07).** 25개 앱 전부 matplotlib→인라인 SVG로
> 전환하고 numpy를 stdlib로 대체했다. 아래 그림의 예상치가 실측으로 재현됐다.

![matplotlib → 인라인 SVG 전환 결과 — 모듈당 첫 방문 페이로드가 34.9 MB에서 22.6 MB로 −35% 감소, matplotlib·numpy 의존 제거 방법과 한글 폰트 깨짐 해소](images/shinylive-optimization-result.svg)

**달성 결과(실측):** 모듈당 페이로드 **34.9 MB → 22.6 MB (−35%)**, 25/25 앱
matplotlib·numpy import 0, 한글 축 라벨 깨짐 해소. 규약은 [CLAUDE.md](CLAUDE.md)에 기록.
(아래 §1의 개략치 그림은 초기 추정으로, 실측 34.9 MB보다 작게 잡혔다 — §1.1 실측 보정 참조.)
